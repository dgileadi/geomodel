# -*- coding: utf-8 -*-
"""
Created on Thu Sep 06 22:25:49 2012

"""

from geomodel import GeoModel, geocell, default_cost_function, DEBUG
import geomath
from google.appengine.ext import ndb
import util
import logging

class GeoModel(GeoModel):

    @classmethod
    @ndb.tasklet
    def bounding_box_fetch_async(cls, query, bbox, limit=1000,
                         cost_function=None):
        ...
        if DEBUG:
            logging.info('query_geocells %s', query_geocells)
            logging.info('limit %s', limit)
        if query_geocells:
            logging.info('Searching %s', query_geocells)
            query = query.filter(cls.location_geocells.IN(query_geocells))
            
            q_results = yield query.fetch_async(limit)
            for entity in q_results:
                logging.info('Entity %s', entity.key)
                ...
                logging.info('location lat %s, lon %s ', entity.location.lat, entity.location.lon  )
                logging.info('box %s, ',bbox)

        if DEBUG:
            logging.info('bbox_async query looked in %d geocells' % len(query_geocells))
    
        raise ndb.Return(results)    

    @classmethod
    @ndb.tasklet
    def proximity_fetch_async(cls, query, center, max_results=10, max_distance=0):
        ...
        while cur_geocells:
            ...
            new_results = yield temp_query.fetch_async(100)
            if DEBUG:
        ...
        raise ndb.Return( [entity for (entity, dist) in results[:max_results]
